...
class Transport:
    ...
    def training_losses(
        ...
        if loss_weight_mask is not None:
            loss_weight_mask = loss_weight_mask.to(device=model_output.device, dtype=model_output.dtype)
        if self.model_type == ModelType.VELOCITY:
            residual = (model_output - ut) ** 2
            if loss_weight_mask is not None:
                # Calculate global factor: total elements / sum of mask
                num_elements = loss_weight_mask.numel()
                mask_sum = loss_weight_mask.sum()
                global_factor = num_elements / mask_sum if mask_sum > 0 else 1.0
                residual = residual * loss_weight_mask * global_factor
            terms['loss'] = mean_flat(residual) 
            ...
            if self.model_type == ModelType.NOISE:
                residual = weight * ((model_output - x0) ** 2)
            else:
                residual = weight * ((model_output * sigma_t + x0) ** 2)
            if loss_weight_mask is not None:
                # Calculate global factor: total elements / sum of mask
                num_elements = loss_weight_mask.numel()
                mask_sum = loss_weight_mask.sum()
