...
def fid_inception_v3():
    ...
    # Prefer local weights on clusters without internet.
    local_w = os.environ.get("FID_WEIGHTS_PATH", "").strip()
    if local_w and os.path.exists(local_w):
        state_dict = torch.load(local_w, map_location="cpu")
    else:
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    # SciPy not available: use a symmetric-eigen fallback.
    # trace(sqrt(sigma1*sigma2)) == trace(sqrt(sigma1_sqrt * sigma2 * sigma1_sqrt))
    warnings.warn("SciPy not found; using eigen-based sqrt fallback for FID. This may be slower.", RuntimeWarning)
    sigma1 = (sigma1 + sigma1.T) * 0.5
    sigma2 = (sigma2 + sigma2.T) * 0.5
    w1, v1 = np.linalg.eigh(sigma1)
    w1 = np.clip(w1, a_min=eps, a_max=None)
    sigma1_sqrt = (v1 * np.sqrt(w1)).dot(v1.T)
    m = sigma1_sqrt.dot(sigma2).dot(sigma1_sqrt)
    m = (m + m.T) * 0.5
    w = np.linalg.eigvalsh(m)
    w = np.clip(w, a_min=0.0, a_max=None)
    tr_covmean = float(np.sum(np.sqrt(w)))
