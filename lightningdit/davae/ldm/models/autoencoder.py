...
class AutoencoderKL(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # log grad norms for parameters optimized by ae_opt before stepping
        ...
        # log grad norms for parameters optimized by disc_opt before stepping
        self._log_optimizer_grad_norms(disc_opt, prefix="disc/")
        disc_opt.step()

    def _log_optimizer_grad_norms(self, optimizer, prefix=""):
        """Log per-parameter gradient L2 norm (normalized by numel) for params in the given optimizer.

        Only parameters that are part of the optimizer's param groups and that have gradients
        will be logged. Metrics are emitted with keys like `grad_norm/<prefix><param_name>`.
        """
        # Build a mapping from parameter id to its dotted name for fast lookup.
        ...
        for group in optimizer.param_groups:
            for param in group.get("params", []):
                ...
                # Normalize by number of elements to keep scale comparable across tensors
