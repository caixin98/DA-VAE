...
class LightningDiTBlock(nn.Module):
    ...
    def forward(self, x, c, feat_rope=None):
        # torch.compile disabled to avoid checkpoint tensor mismatch
        if self.wo_shift:
...
class LightningDiT(nn.Module):
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale, cfg_interval=None, cfg_interval_start=None):
        ...
        # Guidance is applied to the model's primary prediction channels; everything beyond
        # `in_channels` (e.g. optional sigma or auxiliary outputs) is left untouched.
        # To reproduce the original public checkpoints exactly, swap this line back to the
        # three-channel variant:
    ...
    def extract_block_outputs(
            self,
            x: torch.Tensor,
            t: torch.Tensor,
            y: torch.Tensor,
            return_token_states: bool = True,
            return_image_predictions: bool = False,
            detach: bool = True,
            to_cpu: bool = False,
        ):
        """
        Run a forward pass and collect intermediate outputs after each Transformer block.

        Args:
            x: Input tensor of shape (N, C, H, W), typically VAE latents.
            t: Timestep tensor of shape (N,).
            y: Class label tensor of shape (N,).
            return_token_states: If True, return token features (N, T, D) after each block.
            return_image_predictions: If True, map each block's tokens through the final head and unpatchify to (N, C, H, W).
            detach: If True, detach returned tensors from the computation graph.
            to_cpu: If True, move returned tensors to CPU.

        Returns:
            A dict with optional keys:
                - 'token_states': List[Tensor] of length = depth, each (N, T, D)
                - 'image_predictions': List[Tensor] of length = depth, each (N, C, H, W)
        """
        # Embed inputs and conditioning
        tokens = self.x_embedder(x) + self.pos_embed  # (N, T, D)
        t_emb = self.t_embedder(t)                    # (N, D)
        y_emb = self.y_embedder(y, self.training)     # (N, D)
        c = t_emb + y_emb                             # (N, D)

        token_states = [] if return_token_states else None
        image_predictions = [] if return_image_predictions else None
        ...
        for block in self.blocks:
            ...
            if return_token_states:
                ...
                if to_cpu:
                    feat = feat.cpu()
                token_states.append(feat)

            if return_image_predictions:
                pred = self.final_layer(tokens, c)          # (N, T, p*p*out_c)
                pred_img = self.unpatchify(pred)            # (N, out_c, H, W)
                if self.learn_sigma:
                    pred_img, _ = pred_img.chunk(2, dim=1)
                if detach:
                    pred_img = pred_img.detach()
                if to_cpu:
                    pred_img = pred_img.cpu()
                image_predictions.append(pred_img)

        out = {}
        if return_token_states:
            out['token_states'] = token_states
        if return_image_predictions:
            out['image_predictions'] = image_predictions
        return out
