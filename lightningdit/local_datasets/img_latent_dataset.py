...
class ImgLatentDataset(Dataset):
    def __init__(self, data_dir, latent_norm=True, latent_multiplier=1.0, latent_norm_channels=None):
        self.data_dir = data_dir
        self.latent_norm = latent_norm
        self.latent_multiplier = latent_multiplier
        if latent_norm_channels is None:
            self.latent_norm_channels = None
        else:
            try:
                channels = int(latent_norm_channels)
            except (TypeError, ValueError) as exc:
                raise ValueError("latent_norm_channels must be an integer when provided") from exc
            if channels <= 0:
                channels = None
            self.latent_norm_channels = channels
    ...
    def __getitem__(self, idx):
        ...
        if self.latent_norm:
            if self.latent_norm_channels is None:
                feature = (feature - self._latent_mean) / self._latent_std
            else:
                k = min(self.latent_norm_channels, feature.shape[1])
                if k > 0:
                    feature[:, :k] = (feature[:, :k] - self._latent_mean[:, :k]) / self._latent_std[:, :k]
