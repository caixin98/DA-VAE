...
class EMAModel:
    ...
    def step(self, parameters: Iterable[torch.nn.Parameter]):
        ...
        for s_param, param in zip(self.shadow_params, parameters):
            if param.requires_grad:
                # Ensure one_minus_decay is on the same device as the parameters
                one_minus_decay_tensor = torch.tensor(one_minus_decay, device=s_param.device, dtype=s_param.dtype)
                s_param.sub_(one_minus_decay_tensor * (s_param - param))
            else:
                s_param.copy_(param)
    ...
    def restore(self, parameters: Iterable[torch.nn.Parameter]) -> None:
        ...
        for c_param, param in zip(self.temp_stored_params, parameters):
            param.data.copy_(c_param.to(param.device).data)
