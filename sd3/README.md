...
│   ├── modules/
│   │   ├── sd3_da_vae.py      # DA-VAE encoder/decoder (core)
│   │   ├── losses.py          # Training losses (reconstruction, alignment, diffusion)
│   │   ├── ema_model.py       # EMA for training
│   │   └── blocks.py          # Diffusion loss MLP
│   ├── sd3_da_vae_wrapper.py  # DA-VAE inference wrapper
...
│   │   ├── trainer.py         # Core training loop (zero-init + loss schedule)
│   │   └── train_sd3_hr.py    # Entry point
│   └── pipeline/              # SD3 inference pipeline (zero-init wrapper)
├── train/                     # Launch scripts and configs for LoRA fine-tuning
├── inference/                 # Inference configs
├── eval/                      # Evaluation (FID, GenEval, CLIP Score)
...
data:
  local_train:
    image_dir: "/path/to/your/dataset"    # synthetic data generated by FLUX using t2i-prompts-3m
  local_eval:
...
- Training data: SAM dataset for DA-VAE; synthetic images generated by [FLUX](https://github.com/black-forest-labs/flux) using prompts from [t2i-prompts-3m](https://huggingface.co/datasets/Lakonik/t2i-prompts-3m) for LoRA fine-tuning
